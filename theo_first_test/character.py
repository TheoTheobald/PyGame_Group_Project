...
class Character(pygame.sprite.Sprite):
    ...
    def getSprites(self, pos): # Creates a 3D array of images for the enemy and playable characters
        ...
        for j in rightLeft:
            ...
            for elem in animationTypes:
                lst = []
                frames = len(os.listdir(f"images/{self.className}/{elem}")) # Third layer into the array, number of images in that animation loop
        ...
        self.image = self.animations[self.facing][self.stance][self.frameIndex] # Sets the image to the first in the starting array
    ...
    def healthBar(self, scrn): # Draws two bars above the characters head, one red, one green, green shortens as health depletes
