...
class BigEnemy(Character):
    ...
    def animate(self):
        ...
        # if self.facing == 1:
        #     f = 10
        self.image = self.animations[self.facing][self.stance][self.frameIndex + f]  # Update image to match current frame
        ...
        if self.frameIndex >= len(self.animations[self.facing][self.stance]):#9:
            if self.stance == 1:
                self.kill()
            self.frameIndex = 0
